...
#结果存储位置
res_file = f"../../../result/{model}/{class_name}/baselines_discriminative/{language}/examined_context_conversationlast_{type}_result.csv"
# 日志文件
log_file = f"../../../result/{model}/{class_name}/baselines_discriminative/{language}/examined_context_conversationlast_{type}_log.txt"
...
for i in tqdm(range(len(df))):
    ...
    conversation_last = modified_conversation.split(":")[-1]
        ...
        if language == 'zh':
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": "{}\n在这种情况下，你赞同以下观点吗？\n{}\n如果你赞同，回复“是”；否则回复“否”。".format(row['context'],conversation_last)},
                ],
            ...
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": "{}\nUnder this circumstance, do you agree with the following statement?\n{}\nIf you agree, then answer 'yes', otherwise answer 'no'.".format(row['context'],conversation_last)},
                ],
                max_tokens=max_tokens,
                n=generate_times,
            )
